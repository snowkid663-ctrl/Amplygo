...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  if (getParticipation(campaign.id, creator.id)) {
    return NextResponse.json({ error: "You already joined this campaign." }, { status: 400 });
  }

  if (campaign.maxCreators && countParticipants(campaign.id) >= campaign.maxCreators) {
    return NextResponse.json({ error: "This campaign has reached its creator limit." }, { status: 400 });
  }

  const participation = joinCampaign(campaign.id, creator.id);
