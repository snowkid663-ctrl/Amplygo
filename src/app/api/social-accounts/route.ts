...
export async function GET() {
  ...
  return NextResponse.json({ accounts: listSocialAccounts(creator.id) });
}
...
export async function POST(req: Request) {
  ...
  connectSocialAccount(creator.id, platform, handle.trim());
...
export async function DELETE(req: Request) {
  ...
  disconnectSocialAccount(creator.id, platform);
  return NextResponse.json({ ok: true });
}
